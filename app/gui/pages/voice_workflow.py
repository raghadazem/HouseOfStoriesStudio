...
# --- "Generate the missing lines in this scene" ------------------------------


class _GenerateMissingDialog(FormDialog):
    """Sequentially runs one single-candidate batch per MISSING line (i.e.
    ready to generate and not already covered by a current candidate —
    see :class:`~app.core.services.voice_production_candidate_service.VoiceProductionCandidateService`),
    on a background GenerationWorker (never blocking the UI thread) —
    never a candidate-review grid of its own; each line's own row still
    opens its normal review flow afterward. No episode-wide equivalent
    exists yet (Milestone 9 Decision 5) — this is scene-scoped only.

    The caller (``_SceneVoiceCard._on_generate_missing``) is responsible
    for computing ``line_ids`` fresh, right before this dialog opens —
    that's what makes repeated use idempotent: a line that already
    succeeded (or already had a reusable candidate) simply never enters
    ...
    def __init__(
        ...
        super().__init__("Generate Missing Lines", icon="🎙️", show_save=False, parent=parent)
        self._ctx = ctx
    ...
    def __init__(
        ...
        *,
        ...
        if has_final_take:
            ...
            # Already has a candidate matching the CURRENT production
            # configuration (voice, model, text, format) -- distinct
            # from Final Take (a human hasn't promoted it yet) and from
            # a plain job_count, which counts historical/superseded
            # attempts too. This is what "Generate Missing" uses to
            # skip this line.
            current_badge = StatusBadge("✓ Current Candidate", "info")
            current_badge.setToolTip(
                "A candidate already matches the current voice/model/text/format. "
                "'Generate Missing' will skip this line. Use 'Generate' below for an "
                "intentional new candidate."
            )
...
class _SceneVoiceCard(QFrame):
    def __init__(
        ...
        *,
        ...
        self._scene = scene
        self._on_changed = on_changed
        # "Missing" = technically ready to generate AND does not already
        # have a candidate matching the current production configuration
        # -- deliberately not the same set as "ready" (see
        # VoiceProductionCandidateService's module docstring): a line can
        # be ready while already done, and must not be regenerated by
        # this bulk action.
        self._missing_lines = [
            line
            for line, is_ready, _msgs, _final, has_candidate, _count, _song in line_rows
            if is_ready and not has_candidate
        ]
        self._reusable_count = sum(
            1 for _line, _ready, _msgs, _final, has_candidate, _count, _song in line_rows if has_candidate
        )
        self._tts_eligible_count = sum(
            1 for _line, _ready, _msgs, _final, _has_candidate, _count, is_song in line_rows if not is_song
        )
        ...
        missing_count = len(self._missing_lines)
        header.addWidget(
            StatusBadge(
                f"{missing_count}/{self._tts_eligible_count} to generate",
                "warning" if missing_count else "success",
            )
        )
        if self._reusable_count:
            header.addWidget(StatusBadge(f"{self._reusable_count} reusable", "info"))
        generate_missing_btn = QPushButton("Generate Missing")
        generate_missing_btn.setEnabled(missing_count > 0)
        generate_missing_btn.clicked.connect(self._on_generate_missing)
        header.addWidget(generate_missing_btn)
        layout.addLayout(header)

        if not line_rows:
            layout.addWidget(QLabel("No dialogue in this scene yet."))
        for line, is_ready, blocking_messages, has_final_take, has_current_candidate, job_count, is_song_scene in line_rows:
            layout.addWidget(
                _DialogueLineRow(
                    ctx, theme, line,
                    is_ready=is_ready, blocking_messages=blocking_messages,
                    has_final_take=has_final_take, has_current_candidate=has_current_candidate,
                    job_count=job_count, is_song_scene=is_song_scene,
                    on_changed=on_changed,
                )
            )

    def _on_generate_missing(self) -> None:
        count = len(self._missing_lines)
        lines_preview = "\n".join(
            f"• {line.speaker_raw}: {line.authored_text[:40]}{'…' if len(line.authored_text) > 40 else ''}"
            for line in self._missing_lines
        )
        if not confirm(
            self, "Generate Missing Lines",
            f"This will make {count} real paid voice-generation call(s) for this scene "
            f"(lines already matching a current candidate are skipped):\n\n{lines_preview}\n\nContinue?",
        ):
            return
        missing_line_ids = [line.id for line in self._missing_lines]
        dialog = _GenerateMissingDialog(self._ctx, self._scene.episode_id, missing_line_ids, parent=self)
        dialog.exec()
        if dialog.changed:
            self._on_changed()
...
class VoiceWorkspacePanel(QWidget):
    ...
    def __init__(self, ctx: ApplicationContext, theme: ThemeManager, parent: QWidget | None = None) -> None:
        ...
        self._summary_row.setSpacing(METRICS.spacing_sm)
    ...
    def refresh(self, episode_id: uuid.UUID) -> None:
        ...
        try:
            with self._ctx.open_session() as session:
                ...
                candidate_service = VoiceProductionCandidateService(config=self._ctx.config)
                card_data = []
                for scene in scenes:
                    ...
                    for line in lines:
                        ...
                        if not is_song_scene and provider_name is not None:
                            candidate_report = candidate_service.evaluate_line(
                                session, line.id, provider_name=provider_name,
                                orchestrator=self._ctx.ai_orchestrator,
                            )
                            has_current_candidate = candidate_report.has_current_candidate
                        job_count = len(
                            self._ctx.generation_job_service.list_jobs(session, dialogue_line_id=line.id)
                        )
                        line_rows.append(
                            (
                                line, is_ready, blocking_messages, has_final_take,
                                has_current_candidate, job_count, is_song_scene,
                            )
                        )
        ...
        self._refresh_summary_row(card_data)
    ...
    def _refresh_summary_row(self, card_data: list[tuple]) -> None:
        """Episode-wide totals, visible before any paid call is made --
        deliberately the same 4-number breakdown
        (total/reusable/needs-generation/blocked) the pre-bulk
        production-plan checkpoint reported for Episode 001, but always
        derived live from current DB state, never hardcoded."""
        ...
        all_rows = [row for _scene, line_rows in card_data for row in line_rows]
        spoken_rows = [row for row in all_rows if not row[6]]  # not is_song_scene
        total = len(spoken_rows)
        reusable = sum(1 for row in spoken_rows if row[4])  # has_current_candidate
        needs_generation = sum(1 for row in spoken_rows if row[1] and not row[4])  # ready and not current
        blocked = total - reusable - needs_generation

        if total == 0:
            return
        self._summary_row.addWidget(StatusBadge(f"{total} spoken", "neutral"))
        self._summary_row.addWidget(StatusBadge(f"{reusable} reusable", "info"))
        self._summary_row.addWidget(
            StatusBadge(f"{needs_generation} need generation", "warning" if needs_generation else "success")
        )
        self._summary_row.addWidget(
            StatusBadge(f"{blocked} blocked", "warning" if blocked else "neutral")
        )
