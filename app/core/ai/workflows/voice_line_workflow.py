"""VoiceLineWorkflow — generates a candidate voice-line audio clip.

Requires ``episode_id``. ``character_version_id`` is optional and, when
given, merges that character's locked prompt fields into the rendered
line — the closest equivalent to a "voice profile" this milestone has;
``CharacterVersion`` carries no dedicated voice-profile field yet, so
which locked fields are actually useful in a voice prompt template is
left to the template author until a real voice provider makes that
concrete.
"""
